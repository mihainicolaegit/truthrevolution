...
d = feedparser.parse('http://www.gandul.info/rss/autor/cristian-tudor-popescu')
...
for post in d.entries:
    if ws.article_freshness(post.published_parsed,3600) is True:
        ...
        if ws.article_exists(post.title,summary,3) == False:
            ...
            db = client[DB]
            db['article'].insert({"date_ttl":datetime.utcnow(),"authId":int(3),
                                  "authName":"Cristian Tudor Popescu","artName":post.title,
                                  "url":post.link,"summary":summary,"content":content,"tags":tags,"date":int(time.time()),
                                  "artDate":datetime.utcnow().strftime("%d%b%Y"),
                                  "authImgUrl" : "https://ws-truthrevolution.rhcloud.com/images/v1/cristian-tudor-popescu.png",
                                  "artSentiment":int(sentiment),"comments":[],"artSource":"gandul.info",
                                  "artCategory":artCategory})

ws.update_articlelistversion(update_returnArticlesListVersion,3)
