...
d = feedparser.parse('http://techcrunch.com/author/mike-butcher/feed/')
...
for post in d.entries:
    if ws.article_freshness(post.published_parsed,3600) is True:
        if ws.article_exists(post.title,BeautifulSoup(post.summary).get_text(),34) == False:
            ...
            for paragraph in BeautifulSoup(ws.query_cookie(post.link)).find_all('p',{'class':'','id':''}):
                if "id=" not in str(paragraph) and "Get every new post delivered to your Inbox." not in str(paragraph):
                    try:
                        ...
                        pass
                    text_dirty = text_dirty+str(paragraph)
            ...
            db = client['ws']
            db['article'].insert({"date_ttl":datetime.utcnow(),"authId":int(34),"authName":"Mike Butcher",
                                  ...
                                  "authImgUrl" : "https://ws-truthrevolution.rhcloud.com/images/v1/mike-butcher.png",
                                  "artSentiment":int(sentiment),"comments":[],"artSource":"techcrunch.com",
                                  "artCategory":artCategory})

ws.update_articlelistversion(update_returnArticlesListVersion,34)
