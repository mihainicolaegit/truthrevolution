...
db = client['ws']
output=db['article'].find({"artSource":{'$regex':'(.com)', '$options': '-i'}})
...
for id in output:
            ...
            data['text']='%s' % id['content'].encode('utf-8')
            data['maxRetrieve'] = '10'
            data['linkedData'] = '0'
            ...
            try:
                ...
                to_send=id['tags'].encode('utf-8','ignore')
                baseurl='https://query.yahooapis.com/v1/public/yql?'
                yql_query = "select * from contentanalysis.analyze where text='%s' and unique='true' " \
                        "and show_metadata='true' and enable_categorizer='true' " \
                        "and related_entities='true' " % to_send.replace('\'','&#39;')
                ...
                try:
                    ...
                    to_send=id['content'].encode('utf-8','ignore')
                    baseurl='https://query.yahooapis.com/v1/public/yql?'
                    yql_query = "select * from contentanalysis.analyze where text='%s' and unique='true' " \
                        "and show_metadata='true' and enable_categorizer='true' " \
                        "and related_entities='true' " % to_send.replace('\'','&#39;')
                    ...
                    try:
                        ...
                        to_send=id['tags'].encode('utf-8','ignore')+tags_english.encode('utf-8','ignore')
                        baseurl='https://query.yahooapis.com/v1/public/yql?'
                        yql_query = "select * from contentanalysis.analyze where text='%s' and unique='true' " \
                                        "and show_metadata='true' and enable_categorizer='true' " \
                                        "and related_entities='true' " % to_send.replace('\'','&#39;')
