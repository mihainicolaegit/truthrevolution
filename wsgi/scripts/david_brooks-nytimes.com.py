...
d = feedparser.parse('http://topics.nytimes.com/top/opinion/editorialsandoped/oped/columnists/davidbrooks/index.html?rss=1')
...
for post in d.entries:
    if ws.article_freshness(post.published_parsed,3600) is True:
        if ws.article_exists(post.title,BeautifulSoup(post.summary).get_text(),19) == False:
            ...
            text_dirty1 = BeautifulSoup(ws.query_cookie(post.link)).findAll('p',{"class":"story-body-text"})
            if "David Brooks and Gail Collins talk between columns every Wednesday" in str(text_dirty1):
                text_dirty = str(text_dirty1[1:])
            else:
                text_dirty = str(text_dirty1)
            ...
            db = client[DB]
            db['article'].insert({"date_ttl":datetime.utcnow(),"authId":int(19),"authName":"David Brooks","artName":post.title,
                                  ...
                                  "authImgUrl" : "https://ws-truthrevolution.rhcloud.com/images/v1/david-brooks.png",
                                  "artSentiment":int(sentiment),"comments":[],"artSource":"nytimes.com",
                                  "artCategory":artCategory})

ws.update_articlelistversion(update_returnArticlesListVersion,19)
