...
d = [feedparser.parse('http://www.romanialibera.ro/rss/opinii-168.xml'),
     ...
     feedparser.parse('http://www.romanialibera.ro/rss/politica-1001.xml'),
     feedparser.parse('http://www.romanialibera.ro/rss/actualitate-162.xml')]
...
while feeds<4 :
    for post in d[feeds].entries:
        if post.author == "Cristian Campeanu (redactia@romanialibera.ro)":
            if ws.article_freshness(post.published_parsed,3600) is True:
                if ws.article_exists(post.title,post.summary,22) == False:
                    ...
                    db = client[DB]
                    db['article'].insert({"date_ttl":datetime.utcnow(),"authId":int(22),"authName":"Cristian C\xc3\xa2mpeanu",
                                          "artName":post.title,"url":post.link,"summary":post.summary,
                                          "content":content,"tags":tags[:-2],"date":int(time.time()),
                                          "artDate":datetime.utcnow().strftime("%d%b%Y"),
                                          "authImgUrl" : "https://ws-truthrevolution.rhcloud.com/images/v1/cristian-campeanu.png",
                                          "artSentiment":int(sentiment),"comments":[],"artSource":"romanialibera.ro",
                                          "artCategory":artCategory})

    feeds=feeds+1

ws.update_articlelistversion(update_returnArticlesListVersion,22)
