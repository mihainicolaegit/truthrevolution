...
while  numberoflinks<1:
    link=ws.extract_link_revista22('http://www.revista22.ro/autor-vladimir-tismaneanu-1',numberoflinks)
    ...
    if ws.article_exists(title,summary,5) == False:
        ...
        db = client[DB]
        db['article'].insert({"date_ttl": datetime.utcnow(), "authId": int(5), "authName": "Vladimir Tism\xc4\x83neanu",
                              "artName": title, "url": link, "summary": summary, "content": content,
                              "tags": tags[:-2], "date": int(time.time()),
                              "artDate": datetime.utcnow().strftime("%d%b%Y"),
                              "authImgUrl": "https://ws-truthrevolution.rhcloud.com/images/v1/vladimir-tismaneanu.png",
                              "artSentiment": int(sentiment), "comments": [], "artSource": "revista22.ro",
                              "artCategory": artCategory})
    numberoflinks+=1
ws.update_articlelistversion(update_returnArticlesListVersion,5)
