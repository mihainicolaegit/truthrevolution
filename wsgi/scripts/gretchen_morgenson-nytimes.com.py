...
DB = 'ws'
d = feedparser.parse('http://topics.nytimes.com/top/reference/timestopics/people/m/gretchen_morgenson/index.html?rss=1')
...
for post in d.entries:
    if ws.article_freshness(post.published_parsed,3600) is True:
        if ws.article_exists(post.title,BeautifulSoup(post.summary).get_text(),26) == False:
            ...
            text_dirty = str(BeautifulSoup(ws.query_cookie(post.link)).findAll('p',{"class":"story-body-text"}))
            ...
            db = client[DB]
            db['article'].insert({"date_ttl":datetime.utcnow(),"authId":int(26),"authName":"Gretchen Morgenson",
                                  ...
                                  "authImgUrl" : "https://ws-truthrevolution.rhcloud.com/images/v1/gretchen-morgenson.png",
                                  "artSentiment":int(sentiment),"comments":[],"artSource":"nytimes.com",
                                  "artCategory":artCategory})

ws.update_articlelistversion(update_returnArticlesListVersion,26)
